...
struct robin_hood_hashmap
{
	...
	static constexpr const int8_t LogTable256[256] = {
#define LT(n) n, n, n, n, n, n, n, n, n, n, n, n, n, n, n, n
	...
	__m256i get_first_needle(uint16_t home_idx, uint64_t hash) const
	{
		...
		m		   = _mm256_or_si256(m, _mm256_set1_epi16(lower));
		return m;
	}

	__m256i get_second_needle(uint16_t home_idx, uint64_t hash) const
	{
		auto lower = hash & 0x7fe | 0x01;
	...
	bool is_empty(uint16_t header) const
	{
		return (header & 0x1) == 0;
	}

	uint32_t get_first_byte_index(const __m256i& v) const
	{
	...
	uint16_t _make_header(const uint16_t distance_from_home, const uint64_t& hash, uint8_t empty) const
	{
		assert(empty == 0 or empty == 1);
		return (distance_from_home << 11) | hash & (0x0000'0000'0000'07fe) | empty;
	}
